        ...
        <div class="mb-6">
            <nav class="text-sm text-gray-500">
                <ol class="list-reset flex items-center space-x-2">
                    ...
                    <li><span class="text-gray-400">></span></li>
                    <li class="text-gray-700 font-semibold">Edit Permintaan</li>
                </ol>
                <h1 class="text-3xl font-bold text-gray-800 mt-2">Edit Permintaan</h1>
            </nav>
        </div>

        <!-- Form Tambah Barang -->
        <div class="p-6 rounded-lg bg-white shadow-lg border border-gray-200">
            <form action="{{ route('gudang.permintaan-barang.update', $permintaanBarang->id) }}" method="POST"
                    ...
                    <select name="petani_id" id="petani_id" required
                        ...
                        @foreach ($petaniList as $petani)
                            <option value="{{ $petani->id }}"
                                {{ old('petani_id', $petani->petani_id) == $petani->petani_id ? 'selected' : '' }}>
                                {{ $petani->user->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="stok_barang_id" id="stok_barang_id" required
                        ...
                        @foreach ($barangList as $baranglist)
                            <option value="{{ $baranglist->id }}"
                                {{ $permintaanBarang->stok_barang_id == $baranglist->id ? 'selected' : '' }}>
